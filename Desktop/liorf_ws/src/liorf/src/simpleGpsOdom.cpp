...
class GNSSOdom : public ParamServer {
 public:
  GNSSOdom(ros::NodeHandle &_nh) {      // gps
    nh = _nh; 

    if (useGPS) {
      gpsSub = nh.subscribe(gpsTopic, 100, &GNSSOdom::GNSSCB, this,
                            ros::TransportHints().tcpNoDelay());
      gpsOdomPub = nh.advertise<nav_msgs::Odometry>("/gps_odom", 100, false);   // 给到mapping模块
      fusedPathPub = nh.advertise<nav_msgs::Path>("/gps_path", 100);    //  可视化
    }
  }
 ...
 private:
  void GNSSCB(const sensor_msgs::NavSatFixConstPtr &msg) {
    ...
    double gps_time = msg->header.stamp.toSec();    // 时间戳
    ...
    if (!initXyz) {
      ROS_INFO("Init Orgin GPS LLA  %f, %f, %f", msg->latitude, msg->longitude,
               msg->altitude);
      gtools.lla_origin_ = lla;       // 初始化原点 坐标
      initXyz = true;
      return;
    }
    ...
    // ROS_INFO("GPS ENU XYZ : %f, %f, %f", enu(0), enu(1), enu(2));       // 北东地

    // sometimes you may get a wrong origin at the beginning
    if (abs(enu.x()) > 10000 || abs(enu.x()) > 10000 || abs(enu.x()) > 10000) {     // 坐标系值异常， 重置原点
