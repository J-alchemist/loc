...
public:
    ...
    ros::Publisher pubLoopConstraintEdge, pubGpsConstraintEdge;
    ...
    std::vector<gtsam::GPSFactor> keyframeGPSfactor;        // gps
    ...
    mapOptimization()
    {
       ...
       // subGPS   = nh.subscribe<sensor_msgs::NavSatFix> (gpsTopic, 200, &mapOptimization::gpsHandler, this, ros::TransportHints().tcpNoDelay());
        ...
        pubGpsConstraintEdge  = nh.advertise<visualization_msgs::MarkerArray>("/liorf/mapping/gps_constraints", 1);
    ...
    void laserCloudInfoHandler(const liorf::cloud_infoConstPtr& msgIn)
    {
        ...
        if (timeLaserInfoCur - timeLastProcessing >= mappingProcessInterval)
        {
            ...
            if (systemInitialized) {
                ...
                publishOdometry();  // odometryFrame and "odom_mapping"
   ...
   /* void gpsHandler(const sensor_msgs::NavSatFixConstPtr& gpsMsg)
    ...
    bool saveMapService(liorf::save_mapRequest& req, liorf::save_mapResponse& res)
    {
      ...
      pcl::io::savePCDFileBinary(saveMapDirectory + "/trajectory.pcd", *cloudKeyPoses3D);       // xyz
      pcl::io::savePCDFileBinary(saveMapDirectory + "/transformations.pcd", *cloudKeyPoses6D);  // xyz+姿态
    ...
    void updateInitialGuess(double fre)
    {
        ...
        if (cloudKeyPoses3D->points.empty())
        {
            ...
            if (useGPS) {
                ...
                if (syncGPS(alignedGPS, timeLaserInfoCur, 1.0 / fre)) {
                    // when we get agigened gps, we can use it directly
        ...
        if (imuType==-1) {       	 // not use IMU， CV-model for predict
...
/*             float arx = (crx*sry*srz*pointOri.x + crx*crz*sry*pointOri.y - srx*sry*pointOri.z) * coeff.x
    ...
    void addGPSFactor(double fre)
    {
        ...
        if (poseCovariance(3,3) < poseCovThreshold && poseCovariance(4,4) < poseCovThreshold)   // poseCovThreshold=25
            return;

        // last gps position
        static PointType lastGPSPoint;
        nav_msgs::Odometry thisGPS;
        static int timestampGood = 0;
        static bool firstComing = true;
        if (syncGPS(thisGPS, timeLaserInfoCur, 1.0 / fre)) {
            std::cout << "Gps timestamp align good: " << ++timestampGood << std::endl;    // for debug

            // GPS too noisy, skip
            float noise_x = thisGPS.pose.covariance[0];
            float noise_y = thisGPS.pose.covariance[7];
            float noise_z = thisGPS.pose.covariance[14];
            if (abs(noise_x) > gpsCovThreshold || abs(noise_y) > gpsCovThreshold)     // 2, 25
                return;

            float gps_x = thisGPS.pose.pose.position.x;
            float gps_y = thisGPS.pose.pose.position.y;
            float gps_z = thisGPS.pose.pose.position.z;
            if (!useGpsElevation)       // gps的高度估计 
            {
            ...
            /**************************************/ 
            std::cout << curGPSPoint.x << " " << curGPSPoint.y << " " << curGPSPoint.z << std::endl;
            keyframeGPSfactor.push_back(gps_factor);               // for gps constrat visul   
            cloudKeyGPSPoses3D->points.push_back(curGPSPoint);
            // gpsIndexContainer[gps_factor.key()] = cloudKeyGPSPoses3D->size() - 1;        // lidar_odom对应的gps_odom的idx
            /**************************************/             
            if (keyframeGPSfactor.size() < 10) {      // 累计到20帧可靠的gps，加入去优化
                ROS_INFO("Accumulated good gps factor: %d", keyframeGPSfactor.size());
                // return;
            }
            ...
            if (firstComing) {
                ...
                gtSAMgraph.add(gps_factor);     // 加入图优化
                gpsIndexContainer[gps_factor.key()] = cloudKeyGPSPoses3D->size() - 1; 
            }
    ...
    bool syncGPS(nav_msgs::Odometry &aligedGps, double timestamp, double eps_cam) {
        bool hasGPS = false;
    ...
    void saveKeyFramesAndFactor()
    {
        ...
        poseCovariance = isam->marginalCovariance(isamCurrentEstimate.size()-1);    // 协方差
    ...
    void publishOdometry()
    {
        ...
        laserOdometryROS.header.frame_id = odometryFrame;       // if imuType==-1, odometryFrame = mapFrame;
...
int main(int argc, char** argv)
{
    ...
    common_lib_ = std::make_shared<CommonLib::common_lib>(MO.mode);
