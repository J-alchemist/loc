...
enum class SensorType { OTHERS, ROBOSENSE};
...
public:
    ...
    ParamServer()
    {
        nh.param<std::string>("/robot_id", robot_id, "my_robot");
        ...
        nh.param<std::string>("liorf/mode", mode, "mapping");

        std::string sensorStr;
        nh.param<std::string>("liorf/sensor", sensorStr, "");
        if (sensorStr == "robosense")
        {
            sensor = SensorType::ROBOSENSE;
        }
        else if (sensorStr == "Others")
        {
            sensor = SensorType::OTHERS;
        }
        else {
            ROS_ERROR_STREAM(
                "Invalid sensor type (must be either 'robosense' or 'Others'): " << sensorStr);
            ros::shutdown();
        }
        ...
        if (imuType==-1) {      // add 2023.3.27

            odometryFrame = mapFrame;
        }
